...
`   , [token]);
    ...
    if(user){
      ...
      historic.rows.map(n => n.created_at = dayjs(n.created_at).format('DD/MM'));
      let total = 0;
      let add = historic.rows.filter(n => n.name === "entrada").map(n => n.valor);
      let remove = historic.rows.filter(n => n.name === "saída").map(n => n.valor);
      total = add.reduce((total, number) => total + number, 0) - remove.reduce((total, number) => total + number, 0)

      historic.rows.map(n => n.valor = (n.valor/100).toFixed(2).replace(".", ","));
      const response = {
        total: (total/100).toFixed(2).replace(".", ","),
        financial_events: historic.rows
      }
